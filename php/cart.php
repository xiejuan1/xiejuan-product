<?php 	
	...
	if ($action == "load") { // 加载用户的购物车信息
		$sql = "SELECT cid, uid, pid, title, price, amount, img FROM mall_carts WHERE uid='$uid' and status=0";
		...
		foreach ($products as $key => $value) {
			...
			if ($row = mysql_fetch_array($result)) {
				if ($row[0] == 0) {
					add($uid, $pid, $value->title, $value->price, $value->amount, $value->img, "combine");
				} else {
		...
		$amount = $_GET["amount"];
		$img = $_GET["img"];
		add($uid, $pid, $title, $price, $amount, $img, "add");
	...
	function add($uid, $pid, $title, $price, $amount, $img, $status) {
		$sql = "INSERT INTO mall_carts VALUES (NULL, '$uid', '$pid', '$title', '$price', '$amount', '$img', 0)";
		$result = mysql_query($sql);
		if ($status == "combine") {
			if (!$result)
				die('{"res_code":0, "res_error":"", "res_body":{"status":0, "message":"failed"}}');
		} else {
			if ($result)
				echo '{"res_code":0, "res_error":"", "res_body":{"status":1,"message":"success"}}';
			else
